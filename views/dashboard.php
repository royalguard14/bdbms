...
<div class="row top_tiles" >
  <div class="tile_count col-md-12 col-sm-12">
    <?php if ($_SESSION["role"]['name'] == "BRGY USER"): ?>
      ...
      <?php
    // Initialize necessary variables with default values
      $thisYearBudget = (int) str_replace(',', '', $lastyear['this_year_allocated_budget'] ?? 0);
      $lastYearUnusedBudget = (int) str_replace(',', '', $lastyear['last_year_unused_budget'] ?? 0);
      $totalAvailableBudget = (int) str_replace(',', '', $lastyear['total_available_budget'] ?? 0);

      $totalAllocatedBudget = $buotira ?? 0;
      $totalAllocatedQRFBudget = $totalgetfromQRFBudget ?? 0;
      $totalSpentOnPlans = $totalgetfromAlocatedBudget ?? 0;

    // Total budget (This year + Last year's unused budget)
      $totalBudget = $thisYearBudget + $lastYearUnusedBudget;
    ...
    // Remaining QRF Balance
      $remainingQRF = $qrfBudget - $totalAllocatedQRFBudget;

    // Remaining Plans Balance
      $remainingPlans = $plansBudget - $totalSpentOnPlans;
      ?>
